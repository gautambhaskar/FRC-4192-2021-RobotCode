...
public class UILAuton extends SequentialCommandGroup {
  ...
  public UILAuton(Drivetrain drive, Turret turret, ShootingSystem shooter, Index index, Hood hood, Intake intake, int route) {
    ...
    case 0: // turret side bumpers on line (DSMid)
      addCommands(new SetIntake(intake, false), new ShootingMacro(drive, turret, shooter, index, hood, 3, 0, 2000, false), 
          new DriveSetDistance(drive, 3 * drivePID.feetToRotations));
      break;
    case -1: // the 1 on bumpers on line (DSLeft)
      addCommands(new SetIntake(intake, false), new ShootingMacro(drive, turret, shooter, index, hood, 3, 20, 2000, false),
          new DriveAndIntake(drive, intake, 16 * drivePID.feetToRotations, false), new DriveSetDistance(drive, -7 * drivePID.feetToRotations), new ShootingMacro(drive, turret, shooter, index, hood, 3, 0, 2000, false));
      break;
