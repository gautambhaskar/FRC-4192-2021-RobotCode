...
// public static List<SwitchedCameraConfig> switchedCameraConfigs = new
// ArrayList<>();
...
// System.out.println("Starting switched camera '" + config.name + "' on " +
// config.key);
// MjpegServer server =
// CameraServer.getInstance().addSwitchedCamera(config.name);
...
// if (cameras.size() >= 1) {
// VisionThread visionThread = new VisionThread(cameras.get(0), new
// MyPipeline(), pipeline -> {
